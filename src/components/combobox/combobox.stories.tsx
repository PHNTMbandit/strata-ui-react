...
export default {
	...
	parameters: {
		docs: {
			subtitle:
				"A Combobox component for selecting options from a dropdown list.",
			description: {
				component:
			...
			if (isPending) {
				return (
					<div className="flex items-center gap-3xs">
						<SpinnerGapIcon
							aria-hidden
							className="size-xs animate-spin"
							weight="bold"
						/>
						Searching…
					</div>
						...
						{(user: DirectoryUser) => (
							<ComboboxItem key={user.id} value={user}>
								<div className="flex flex-col gap-1">
									<div className="font-medium text-[0.95rem]">{user.name}</div>
									<div className="flex flex-wrap gap-3 text-[0.8125rem] text-on-surface-variant">
