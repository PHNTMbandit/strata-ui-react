...
export const ComboboxItem = ({ className, children, ref, ...props }: ComboboxItemProps) => {
  return (
    <BaseCombobox.Item
      className={cn(
        'mr-2xs grid cursor-default grid-cols-[1fr_0.75rem] items-center gap-xs rounded-md border border-transparent py-3xs pr-2xs pl-xs style-text-default-0 leading-md outline-none select-none group-data-[side=none]:pr-xs group-data-[side=none]:leading-lg first-of-type:mt-2xs last-of-type:mb-2xs hover:cursor-pointer hover:border-brand-outline data-highlighted:relative data-highlighted:z-0 data-highlighted:border-brand-outline data-highlighted:bg-brand-container data-highlighted:text-on-brand-container data-highlighted:before:absolute data-highlighted:before:inset-x-3xs data-highlighted:before:inset-y-[0px] data-highlighted:before:z-[-1] data-highlighted:before:rounded-sm data-selected:bg-brand data-selected:text-on-surface-inverse dark:data-selected:text-on-surface pointer-coarse:py-xs',
        className,
      )}
      ref={ref}
      {...props}
