	...
	"flex items-center justify-center transition-all hover:cursor-pointer text-on-surface-variant not-disabled:active:inset-shadow-xs not-disabled:not-active:hover:shadow-xs disabled:opacity-70 border border-transparent",
	{
		variants: {
			size: {
				...
				small:
					"h-md px-3xs gap-3xs rounded-sm style-text-default--2 [&>svg]:size-xs",
				medium:
					"h-lg px-2xs gap-2xs rounded-md style-text-default--1 [&>svg]:size-sm",
				large:
					"h-xl px-xs gap-xs rounded-lg style-text-default-0 [&>svg]:size-md",
			},
			tone: {
				brand:
