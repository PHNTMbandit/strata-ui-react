...
export const AnchoredToastItem = ({ className, ref, toast, ...props }: AnchoredToastItemProps) => {
  return (
    <BaseToast.Root
      className={cn(
        'group flex w-max origin-(--transform-origin) flex-col rounded-md border border-outline bg-surface-container px-sm py-2xs shadow-lg outline outline-outline-variant transition-[transform,scale,opacity] focus-visible:outline-1 focus-visible:-outline-offset-1 data-ending-style:scale-90 data-ending-style:opacity-0 data-starting-style:scale-90 data-starting-style:opacity-0',
        className,
      )}
      ref={ref}
      toast={toast}
