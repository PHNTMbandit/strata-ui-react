...
export const Progress = ({ className, children, ref, ...props }: ProgressProps) => {
  let activeIndex = -1
  let currentIndex = -1

  const getDisplayName = (child: React.ReactElement): string | undefined => {
    const childType = child.type as string | React.ComponentType<unknown> | undefined
  ...
  const processChildren = (child: React.ReactNode): React.ReactNode => {
    ...
    if (displayName === 'ProgressIndicator') {
      indicatorIndex++
      const shouldBeCompleted = activeIndex !== -1 && indicatorIndex < activeIndex
  ...
  return (
    <div
      className={cn('flex w-full items-center justify-center gap-2xs', className)}
      ref={ref}
      {...props}
    >
      {processedChildren}
